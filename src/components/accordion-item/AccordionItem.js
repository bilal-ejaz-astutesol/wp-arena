...
export default function AccordionItem({isOpen, isOpenBlog, onClick,data }) {
  ...
  return (
    ...
    <div className="wpa-accordion-title wpa-flex wpa-space-between" onClick={onClick}>
     <div className="wpa-accordion-title-wrapper wpa-flex ">
     <div className="logo"><img src={data.logo} alt={data.logo} /> </div>  <h3> {data.title}</h3> 
     </div>
      <span>{isOpen ? '' : '+'}</span>
    </div>
    {isOpen && <div className={`wpa-accordion-content ${isOpen ? 'open' : ''}`}>
        <CouponsAndDeals showDis={isShowDis}  butonLabel = "false" />
    </div>}
    {isOpenBlog && <div className={`wpa-accordion-content ${isOpenBlog ? 'open' : ''}`}>
