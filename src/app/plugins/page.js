...
const Plugins = ({ ButtonText, isShowBreadCrumb = true, IshwoPluginContent = true, IsShowSearchBar = true }) => {
  ...
  const { type } = router.query; // Get the category slug from the URL
                ...
                <div className="content-right wpa-flex wpa-gap-40">
                  <div>
                    <div className='wpa-blog-list-thumbnail'>
                      <Image src={post.featuredImage?.node?.sourceUrl} alt={post.featuredImage?.node?.altText || post.title} />
                    </div>
                  </div>
                  <div className='wpa-blogs-details wpa-flex wpa-h3-font-size'>
                    <div>
                      <div className='wpa-blog-list-posted-by wpa-paragraph-text wpa-font-weight-600'>
                        <span>Recent updated on {new Date(post.date).toLocaleDateString()} by {post.author?.node?.name}
                          <i className='wpa-share-icon wpa-message-icon'><Image src={wpamessage} alt='share blog post icon' /></i>
                        </span>
                      </div>
